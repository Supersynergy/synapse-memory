...
pub enum CypherOp {
    ...
    /// MATCH p = SHORTEST PATH (a)-->(b)
    ShortestPath { from_id: i64, to_id: i64, max_depth: usize },
    ...
    Neighbors { node_id: i64, top_k: usize, rel_filter: Option<String> },
    /// CREATE (a)-[:REL {weight:w}]->(b)
    Create { from_id: i64, to_id: i64, rel: String, weight: f64 },
}
...
pub fn parse_cypher(input: &str) -> Result<CypherQuery> {
    ...
    if lower.starts_with("call graph.pagerank") {
        ...
        return Ok(CypherQuery {
            op: CypherOp::PageRank { top_n, damping: 0.85 },
            raw: s.into(),
        });
    }
    ...
    if lower.contains("shortest path") || lower.contains("shortestpath") {
        let ids = extract_two_ids(&lower)
            .ok_or_else(|| GraphError::CypherParse("SHORTEST PATH needs id(a)=N, id(b)=M".into()))?;
        return Ok(CypherQuery {
            op: CypherOp::ShortestPath { from_id: ids.0, to_id: ids.1, max_depth: 10 },
            raw: s.into(),
        });
    }
    ...
    if lower.starts_with("create") {
        ...
        return Ok(CypherQuery {
            op: CypherOp::Create { from_id: ids.0, to_id: ids.1, rel, weight },
            raw: s.into(),
        });
    }
    ...
    if lower.starts_with("match") {
        ...
        if max_depth == 1 {
            return Ok(CypherQuery {
                op: CypherOp::Neighbors { node_id: start_id, top_k: limit, rel_filter: rel },
                raw: s.into(),
            });
        }
        return Ok(CypherQuery {
            op: CypherOp::Traverse { start_id, max_depth, rel_filter: rel, limit },
            raw: s.into(),
        });
    }
...
fn extract_first_id(s: &str) -> Option<i64> {
    ...
    let num: String = after_eq.chars().take_while(|c| c.is_ascii_digit() || *c == '-').collect();
    num.parse().ok()
}
...
fn extract_two_ids(s: &str) -> Option<(i64, i64)> {
    ...
    while ids.len() < 2 {
        ...
        if let Ok(n) = num.parse::<i64>() { ids.push(n); }
        rest = &rest[eq_pos + 1..];
    }
    Some((ids[0], ids[1]))
}
...
mod tests {
    ...
    fn parse_traverse_with_depth() {
        let q = parse_cypher("MATCH (a)-[:CITES*1..3]->(b) WHERE id(a) = 99 RETURN b LIMIT 20").unwrap();
        match q.op {
    ...
    fn parse_shortest_path() {
        let q = parse_cypher("MATCH p = SHORTEST PATH ((a)-->(b)) WHERE id(a) = 1 AND id(b) = 7").unwrap();
        match q.op {
    ...
    fn parse_create_edge() {
        let q = parse_cypher("CREATE (a)-[:CITES {weight:0.7}]->(b) WHERE id(a)=10 AND id(b)=20").unwrap();
        match q.op {
            CypherOp::Create { from_id, to_id, rel, weight } => {
