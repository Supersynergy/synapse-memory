...
fn bars_to_json(bars: &[Bar]) -> String {
    let rows: Vec<String> = bars
        ...
        .map(|b| {
            format!(
                r#"{{"ts":{},"open":{:.4},"high":{:.4},"low":{:.4},"close":{:.4},"volume":{:.2}}}"#,
...
fn abi_rust_only_hash_agreement() {
    ...
    let sq_bars: Vec<Bar> =
        {
            ...
            stmt.query_map(rusqlite::params![mid_start, mid_end], |r| {
                Ok(Bar {
                    ts: r.get::<_, i64>(0)?,
    ...
    let dk_bars: Vec<Bar> =
        {
            ...
            stmt.query_map(duckdb::params![mid_start, mid_end], |r| {
                Ok(Bar {
                    ts: r.get::<_, i64>(0)?,
                    open: r.get::<_, f64>(1)? as f32,
                    high: r.get::<_, f64>(2)? as f32,
    ...
    // Hilbert-sort reorders bars internally; normalise to ts-order before hashing.
    let mut smx_sorted = smx_bars.clone();
    smx_sorted.sort_by_key(|b| b.ts);
    let mut sq_sorted = sq_bars.clone();
    sq_sorted.sort_by_key(|b| b.ts);
    let mut dk_sorted = dk_bars.clone();
    dk_sorted.sort_by_key(|b| b.ts);
    let smx_json = bars_to_json(&smx_sorted);
    let sq_json = bars_to_json(&sq_sorted);
    let dk_json = bars_to_json(&dk_sorted);
...
fn abi_cross_lang_stub_documented() {
    ...
    eprintln!(
        "cross-lang ABI status: rust-only (python/TS stubs in tests/ — see py_query.py, ts_query.ts)"
    );
}
