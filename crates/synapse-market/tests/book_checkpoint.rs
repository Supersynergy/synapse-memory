...
fn checkpoint_every_1000_events() {
    ...
    let batch: Vec<BookEvent> = (0..1000)
        .map(|i| BookEvent {
    ...
    assert_eq!(
        store.n_checkpoints(),
        2,
        "expected 2 checkpoints after 1000 events"
    );

    // Append another 1000
    let batch2: Vec<BookEvent> = (0..1000)
        .map(|i| BookEvent {
    ...
    assert_eq!(
        store.n_checkpoints(),
        3,
        "expected 3 checkpoints after 2000 events"
    );
...
fn checkpoint_partial_batch() {
    ...
    assert_eq!(
        store.n_checkpoints(),
        1,
        "only initial checkpoint for <1000 events"
    );
