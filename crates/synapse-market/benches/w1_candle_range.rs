...
fn setup_sqlite(dir: &TempDir) -> Connection {
    ...
    let _: Vec<i64> = conn
        ...
        .unwrap()
        .query_map(
            rusqlite::params![BASE_TS, BASE_TS + BARS_PER_TICKER as i64 * 900],
            |r| r.get(0),
        )
        .unwrap()
        .map(|r| r.unwrap())
...
fn bench_smx(path: &std::path::Path) -> usize {
    let mut s = Series::open(path).unwrap();
    let bars = s
        .range(BASE_TS..BASE_TS + BARS_PER_TICKER as i64 * 900)
        .unwrap();
    bars.len()
}
...
fn main() {
    ...
    eprintln!(
        "Setting up SQLite ({} tickers × {} bars)...",
        TICKERS, BARS_PER_TICKER
    );
    ...
    println!(
        "{:<25} {:>12} {:>12} {:>12}",
        "SQLite-WITHOUT-ROWID", sql_p50, sql_p95, sql_mean
    );
    println!(
        "{:<25} {:>12} {:>12} {:>12}",
        "Synapse-X (open-each)", smx_p50, smx_p95, smx_mean
    );
    ...
    println!(
        "{:<25} {:>12.3} {:>12.3} {:>12.3}",
        "Synapse-X (held-handle)", h_p50_us, h_p95_us, h_mean_us
    );
    println!();
    let speedup_held_p50 = sql_p50 as f64 / h_p50_us.max(0.001);
    let speedup_held_mean = sql_mean as f64 / h_mean_us.max(0.001);
    println!(
        "Speedup p50  (open-each)  : {:.1}×   mean : {:.1}×",
        speedup_p50, speedup_mean
    );
    println!(
        "Speedup p50  (held-handle): {:.1}×   mean : {:.1}×",
        speedup_held_p50, speedup_held_mean
    );
    let gate_held = if speedup_held_p50 >= 10.0 {
        "GREEN ✅"
    } else if speedup_held_p50 >= 3.0 {
        "ORANGE ⚠️"
    } else {
        "RED ❌"
    };
