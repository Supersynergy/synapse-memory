...
fn test_drought_buy() {
    ...
    let id = engine.register(Pattern::DroughtBuy {
        quiet_days: 30,
        min_value: 100_000.0,
    });

    // First buy — no match yet (no prior reference)
    let ev1 = Event::InsiderBuy {
        value_usd: 200_000.0,
        ts: 0,
        ticker: "AAPL".into(),
    };
...
fn test_drought_buy_too_soon() {
    let mut engine = FsmEngine::new();
    engine.register(Pattern::DroughtBuy {
        quiet_days: 30,
        min_value: 100_000.0,
    });
...
fn test_insider_cluster() {
    let mut engine = FsmEngine::new();
    let id = engine.register(Pattern::InsiderCluster {
        k: 3,
        window_days: 14,
    });
...
fn test_volume_spike() {
    let mut engine = FsmEngine::new();
    let id = engine.register(Pattern::VolumeSpike {
        multiplier: 2.0,
        window_bars: 5,
    });
...
fn test_and_composition() {
    let p = Pattern::And(
        Box::new(Pattern::InsiderCluster {
            k: 2,
            window_days: 7,
        }),
        Box::new(Pattern::VolumeSpike {
            multiplier: 1.5,
            window_bars: 3,
        }),
    );
