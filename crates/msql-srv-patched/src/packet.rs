...
impl<W: Read + Write> Write for PacketConn<W> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        // MySQL packets are limited to U24_MAX bytes per segment.
        // Write in a loop so callers always see all bytes consumed
        // and large payloads are split across multiple packets automatically.
        let mut remaining = buf;
        while !remaining.is_empty() {
            let space = U24_MAX - self.to_write.len();
            let take = remaining.len().min(space);
            self.to_write.extend(&remaining[..take]);
