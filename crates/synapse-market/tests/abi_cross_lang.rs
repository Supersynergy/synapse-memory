...
fn abi_rust_only_hash_agreement() {
    ...
    let mut smx_sorted = smx_bars.clone(); smx_sorted.sort_by_key(|b| b.ts);
    let mut sq_sorted  = sq_bars.clone();  sq_sorted.sort_by_key(|b| b.ts);
    let mut dk_sorted  = dk_bars.clone();  dk_sorted.sort_by_key(|b| b.ts);
