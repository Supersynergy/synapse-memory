...
SessionStart hook runtime ~20ms (socket via synx). No perceivable delay.

## Why not MCP?

MCP adds stdio-JSON-RPC hop (~5ms). Direct socket via synx is the same latency with native shell ergonomics — and works even if MCP client not loaded.
