...
impl BookSnapshot {
    fn apply(&mut self, ev: &BookEvent) {
        ...
        match ev.op {
            Op::Delete => { slot.0 = 0.0; slot.1 = 0.0; }
            Op::Insert | Op::Update => { slot.0 = ev.px; slot.1 = ev.qty; }
        }
    }

    pub fn best_bid(&self) -> f32 {
        self.bids.iter().filter(|&&(p, q)| p > 0.0 && q > 0.0)
            .map(|&(p, _)| p).fold(f32::NEG_INFINITY, f32::max)
    }

    pub fn best_ask(&self) -> f32 {
        self.asks.iter().filter(|&&(p, q)| p > 0.0 && q > 0.0)
            .map(|&(p, _)| p).fold(f32::INFINITY, f32::min)
    }
...
impl BookStore {
    ...
    fn load_index(&mut self) -> Result<()> {
        ...
        while pos < file_len {
            ...
            let mut tag = [0u8; 4];
            if self.file.read_exact(&mut tag).is_err() { break; }
            if &tag == b"CKPT" {
    ...
    fn write_checkpoint(&mut self, snap: &BookSnapshot, ts: i64, event_index: usize) -> Result<()> {
        ...
        self.file.write_all(&(snap_bytes.len() as u32).to_le_bytes())?;
    ...
    pub fn append(&mut self, events: &[BookEvent]) -> Result<()> {
        ...
        while !remaining.is_empty() {
            ...
            // Pack events
            let packed: Vec<PackedEvent> = chunk.iter()
            ...
            if idx % EVENTS_PER_CHECKPOINT == 0 {
    ...
    fn build_snapshot_at_idx(&mut self, target_event_idx: usize) -> Result<BookSnapshot> {
        ...
        let cp_idx = self.checkpoints.partition_point(|c| c.event_index <= target_event_idx)
            .saturating_sub(1);
    ...
    pub fn replay_at(&mut self, ts: i64) -> Result<BookSnapshot> {
        ...
        let cp_idx = self.checkpoints.partition_point(|c| c.ts <= ts).saturating_sub(1);
        if self.checkpoints.is_empty() {
    ...
    pub fn events_between(&mut self, start: i64, end: i64) -> Result<Vec<BookEvent>> {
        if self.checkpoints.is_empty() { return Ok(vec![]); }

        let cp_idx = self.checkpoints.partition_point(|c| c.ts <= start).saturating_sub(1);
        let cp = &self.checkpoints[cp_idx];
        let cp_ts = cp.ts;
        ...
        while pos < file_len {
            ...
            if &tag == b"EVTS" {
                ...
                for chunk in all_packed.chunks_exact(12) {
                    let packed = unsafe { &*(chunk.as_ptr() as *const PackedEvent) };
                    let ev = packed.decode(seg_base_ts, seg_base_px);
                    if ev.ts > end { past_end = true; break; }
                    if ev.ts >= start { result.push(ev); }
                }
                if past_end { break; }
                pos += 4 + 4 + (count * 12) as u64;
            } else if &tag == b"CKPT" {
    ...
    pub fn n_checkpoints(&self) -> usize { self.checkpoints.len() }
}
