...
type ChromaEmbeddingRow = (String, Option<String>, Option<String>, Option<Vec<u8>>);
...
fn main() -> Result<()> {
    ...
    let total = if src.starts_with("qdrant://") {
        ...
        bail!(
            "unknown source scheme — expected qdrant:// | lancedb:// | chroma:// | pinecone:// | weaviate://"
        );
    };
...
fn migrate_chroma(src: &str, store: &mut Store, batch: usize, offset: u64) -> Result<u64> {
    ...
    loop {
        ...
        let rows: Vec<ChromaEmbeddingRow> = stmt
            .query_map(
...
mod tests {
    ...
    fn test_pinecone_missing_key() {
        ...
        unsafe { std::env::remove_var("PINECONE_API_KEY") };
