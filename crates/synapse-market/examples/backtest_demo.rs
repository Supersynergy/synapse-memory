...
use synapse_market::{Market, Order, OrderSide, Strategy, Tick};
...
fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Ingest: {} rows in {}ms  →  {:.0} rows/sec",
        total_rows, ingest_ms, tps
    );
    ...
    m.ingest_news(
        ...
        "AAPL smashes Q4 estimates.",
        &["AAPL", "MSFT"],
    )?;
    m.ingest_news(
        50 * DAY,
        "Tech rally continues",
        "NVDA up 5% on AI demand.",
        &["NVDA", "AMD"],
    )?;
    ...
    assert!(
        similar[0].1 >= 0.0 && similar[0].1 <= 1.001,
    ...
    println!(
        "Backtest 1yr/AAPL: {}ms  ticks={} orders={} pnl={:.2} sharpe={:.3} mdd={:.2}%",
        bt_ms,
        report.ticks,
        report.orders,
        report.pnl,
