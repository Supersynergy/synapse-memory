use synapse_market::store::page::{hilbert_index, encode_page, decode_page, Bar};
...
fn hilbert_index_deterministic() {
    ...
    let h1 = hilbert_index(ts_origin + 100, 105.0, ts_origin, price_origin, ts_scale, price_scale);
    let h2 = hilbert_index(ts_origin + 100, 105.0, ts_origin, price_origin, ts_scale, price_scale);
    assert_eq!(h1, h2, "hilbert_index must be deterministic");

    let h3 = hilbert_index(ts_origin + 200, 110.0, ts_origin, price_origin, ts_scale, price_scale);
...
fn encode_page_hilbert_curve_id_set() {
    ...
    assert_eq!(hdr.hilbert_curve_id, 1, "encoded page must have hilbert_curve_id=1");
    assert_eq!(decoded.len(), bars.len(), "bar count preserved after hilbert sort");
...
fn hilbert_sort_microsanity_10k() {
    ...
    let bars: Vec<Bar> = (0..10_000).map(|i| Bar {
        ts: 1_700_000_000 + i as i64 * 60,
    ...
    let mut keys: Vec<u64> = bars.iter()
        .map(|b| hilbert_index(b.ts, b.close, ts_origin, price_origin, ts_scale, price_scale))
        .collect();
    keys.sort_unstable();
    let elapsed = start.elapsed();

    assert!(elapsed.as_millis() < 10, "10k hilbert sort took {}ms (expected <10ms)", elapsed.as_millis());
