...
if ! command -v synx >/dev/null 2>&1; then
  echo "⚠ synx CLI missing. Install the Synapse CLI as synx"
  exit 1
fi

# Install hook scripts
cp "$(dirname "$0")/hooks/"*.sh "$HOOK_DIR/"
cp "$(dirname "$0")/hooks/"*.py "$HOOK_DIR/" 2>/dev/null || true
chmod +x "$HOOK_DIR"/*.sh "$HOOK_DIR"/*.py 2>/dev/null || true
...
echo ""
echo "Hooks active:"
echo "  SessionStart     → inject budgeted project memory"
echo "  UserPromptSubmit → adaptive semantic memory context"
