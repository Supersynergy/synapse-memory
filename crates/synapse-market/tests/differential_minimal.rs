...
impl Rng {
    ...
    fn next_usize(&mut self, n: usize) -> usize { (self.next_u64() as usize) % n }
    fn next_i64(&mut self, lo: i64, hi: i64) -> i64 { lo + (self.next_u64() as i64).abs() % (hi - lo) }
}

fn gen_bars(rng: &mut Rng, base_ts: i64) -> Vec<Bar> {
    let mut price = 100.0 + rng.next_f64() * 50.0;
    (0..BARS_PER_TICKER).map(|i| {
        ...
        price = close as f64;
        Bar { ts: base_ts + i as i64 * 86400, open, high, low, close, volume }
    }).collect()
}
...
fn differential_minimal() {
    ...
    ).unwrap();
    {
        ...
        for t in &tickers {
            for b in &all_bars[t] {
                stmt.execute(rusqlite::params![t, b.ts, b.open as f64, b.high as f64, b.low as f64, b.close as f64, b.volume as f64]).unwrap();
            }
        }
    }
    ...
    ).unwrap();
    {
        ...
        for t in &tickers {
            for b in &all_bars[t] {
                app.append_row(duckdb::params![t.as_str(), b.ts, b.open as f64, b.high as f64, b.low as f64, b.close as f64, b.volume as f64]).unwrap();
            }
        }
        app.flush().unwrap();
    }
    ...
    for _ in 0..QUERIES {
        ...
        // SQLite
        let mut sql_stmt = sql_conn.prepare_cached(
            "SELECT ts, close FROM candles WHERE ticker=? AND ts>=? AND ts<? ORDER BY ts"
        ).unwrap();
        let sql: Vec<(i64, f64)> = sql_stmt.query_map(rusqlite::params![ticker, ts_a, ts_b], |row| {
            Ok((row.get::<_,i64>(0)?, row.get::<_,f64>(1)?))
        }).unwrap().map(|r| r.unwrap()).collect();

        // DuckDB
        let mut ddb_stmt = ddb.prepare(
            "SELECT ts, close FROM candles WHERE ticker=? AND ts>=? AND ts<? ORDER BY ts"
        ).unwrap();
        let ddb_rows: Vec<(i64, f64)> = ddb_stmt.query_map(duckdb::params![ticker.as_str(), ts_a, ts_b], |row| {
            Ok((row.get::<_,i64>(0)?, row.get::<_,f64>(1)?))
        }).unwrap().map(|r| r.unwrap()).collect();

        // Length check
        if smx.len() != sql.len() || smx.len() != ddb_rows.len() {
            eprintln!("LENGTH MISMATCH ticker={} ts=[{},{}) smx={} sql={} ddb={}",
                ticker, ts_a, ts_b, smx.len(), sql.len(), ddb_rows.len());
            disagreements += 1;
            continue;
        }
        ...
        for i in 0..smx.len() {
            ...
            if max_diff > 1e-6 {
                eprintln!("VALUE DIFF ticker={} ts={} smx_close={} sql_close={} ddb_close={} max_diff={}",
    ...
    assert_eq!(disagreements, 0, "{} 3-way disagreements found", disagreements);
    eprintln!("differential_minimal: {} queries, {} assertions passed", QUERIES, total_assertions);
}
...
fn aggregate_avg_close() {
    ...
    {
        let mut stmt = sql_conn.prepare("INSERT INTO candles VALUES (?,?,?)").unwrap();
        for t in &tickers {
            for b in &all_bars[t] {
                stmt.execute(rusqlite::params![t, b.ts, b.close as f64]).unwrap();
            }
        }
    }
    ...
    // DuckDB ingest
    let ddb = DuckConn::open_in_memory().unwrap();
    ddb.execute_batch("CREATE TABLE candles (ticker VARCHAR, ts BIGINT, close DOUBLE);").unwrap();
    {
        let mut app = ddb.appender("candles").unwrap();
        for t in &tickers {
            for b in &all_bars[t] {
                app.append_row(duckdb::params![t.as_str(), b.ts, b.close as f64]).unwrap();
            }
        }
        app.flush().unwrap();
    }
    ...
    for t in &tickers {
        ...
        let smx_avg = if bars.is_empty() { 0.0 } else {
            bars.iter().map(|b| b.close as f64).sum::<f64>() / bars.len() as f64
        };

        // SQLite avg
        let sql_avg: f64 = sql_conn.query_row(
        ...
        // DuckDB avg
        let mut stmt = ddb.prepare("SELECT AVG(close) FROM candles WHERE ticker=? AND ts>=? AND ts<?").unwrap();
        let ddb_avg: f64 = stmt.query_row(duckdb::params![t.as_str(), base_ts, ts_end], |row| row.get(0)).unwrap();

        let diff1 = (smx_avg - sql_avg).abs();
        let diff2 = (smx_avg - ddb_avg).abs();
        if diff1 > 1e-6 || diff2 > 1e-6 {
            eprintln!("AVG DIFF ticker={} smx={} sql={} ddb={} diff_sql={} diff_ddb={}",
                t, smx_avg, sql_avg, ddb_avg, diff1, diff2);
            disagreements += 1;
        }
    }
    assert_eq!(disagreements, 0, "{} aggregate disagreements", disagreements);
