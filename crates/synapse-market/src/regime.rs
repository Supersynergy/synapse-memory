...
fn compute_features(rows: &[(i64, f64, f64, f64, f64, f64)], idx: usize) -> Option<[f32; FEAT_DIM]> {
    if idx == 0 { return None; }
    let (_, _, h, l, c, vol) = rows[idx];
    let (_, _, _, _, c_prev, vol_prev) = rows[idx - 1];

    let ret_1d = if c_prev != 0.0 { ((c / c_prev) - 1.0) as f32 } else { 0.0 };

    let ret_5d = if idx >= 5 {
        let (_, _, _, _, c5, _) = rows[idx - 5];
        if c5 != 0.0 { ((c / c5) - 1.0) as f32 } else { 0.0 }
    } else { 0.0 };

    let window = &rows[idx.saturating_sub(20)..=idx];
    let returns: Vec<f64> = window.windows(2).map(|w| {
        let c0 = w[0].5; let c1 = w[1].5;
        if c0 != 0.0 { c1 / c0 - 1.0 } else { 0.0 }
    }).collect();
    let mean = returns.iter().sum::<f64>() / returns.len().max(1) as f64;
    let vol_20d = (returns.iter().map(|r| (r - mean).powi(2)).sum::<f64>() / returns.len().max(1) as f64).sqrt() as f32;

    let range_norm = if c != 0.0 { ((h - l) / c) as f32 } else { 0.0 };
    let vol_ratio = if vol_prev > 0.0 { (vol / vol_prev) as f32 } else { 1.0 };
...
pub fn build_all(conn: &Connection, symbol: &str, rows: &[(i64, f64, f64, f64, f64, f64)]) -> Result<()> {
...
pub fn search(conn: &Connection, symbol: &str, date_ts: i64, top_n: usize) -> Result<Vec<(i64, f32)>> {
    let t = table_name(symbol);
    // Check table exists
    ...
    let mut stmt = conn.prepare_cached(&format!(
        "SELECT ts, vec FROM {t} WHERE ts < ?1"
    ))?;
    let mut scores: Vec<(i64, f32)> = stmt.query_map(params![date_ts], |r| {
        let ts: i64 = r.get(0)?;
        let blob: Vec<u8> = r.get(1)?;
        Ok((ts, blob))
    })?.filter_map(|r| r.ok()).map(|(ts, blob)| {
        let v = decode(&blob);
        (ts, cosine(&query, &v))
    }).collect();
