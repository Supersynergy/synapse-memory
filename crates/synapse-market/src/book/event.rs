...
impl PackedEvent {
    pub fn encode(ev: &BookEvent, base_ts: i64, base_px: f32) -> Self {
        ...
        let px_delta = ((ev.px - base_px) * 100.0).round().clamp(i16::MIN as f32, i16::MAX as f32) as i16;
        let qty_raw = (ev.qty * 1000.0).round().clamp(i32::MIN as f32, i32::MAX as f32) as i32;
        let side_op = ((ev.side as u8) << 4) | (ev.op as u8 & 0x0F);
        Self { ts_delta, level: ev.level, side_op, px_delta, qty_raw }
    }

    pub fn decode(&self, base_ts: i64, base_px: f32) -> BookEvent {
        let ts = base_ts + self.ts_delta as i64;
        let side = if (self.side_op >> 4) == 0 { Side::Bid } else { Side::Ask };
        ...
        BookEvent { ts, level: self.level, side, op, px, qty }
    }
}
