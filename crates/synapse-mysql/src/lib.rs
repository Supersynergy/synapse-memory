...
use synapse_libsql::{QueryResult, Store};
use tokio::io::AsyncWrite;
use tokio::net::TcpListener;

const SYNAPSQL_VERSION: &str = "8.0.32-SynapsQL-1.0";
const WIRE_RESULT_CACHE_CAP: usize = 4096;
const WIRE_RESULT_CACHE_SHARDS: usize = 16;
const WIRE_RESULT_CACHE_MAX_ROWS: usize = 128;
const WIRE_RESULT_CACHE_MAX_BYTES: usize = 256 * 1024;

type TextRows = Vec<Vec<String>>;
type TextResult = (Vec<String>, TextRows);
...
fn is_txn_statement(sql: &str) -> bool {
    ...
    lead.eq_ignore_ascii_case("BEGIN")
        || lead.eq_ignore_ascii_case("START")
        || lead.eq_ignore_ascii_case("COMMIT")
        || lead.eq_ignore_ascii_case("END")
        || lead.eq_ignore_ascii_case("ROLLBACK")
        || lead.eq_ignore_ascii_case("SAVEPOINT")
        || lead.eq_ignore_ascii_case("RELEASE")
}

/// Detect `EXPLAIN [ANALYZE] <inner>` and return (inner_sql, analyze).
fn strip_explain(sql: &str) -> Option<(String, bool)> {
    let trimmed = sql.trim().trim_end_matches(';');
    if !starts_ci(trimmed, "EXPLAIN") {
        return None;
    }
    let rest = trimmed[7..].trim_start();
    let (inner, analyze) = if starts_ci(rest, "ANALYZE") {
        (rest[7..].trim().to_owned(), true)
    } else if starts_ci(rest, "QUERY PLAN") {
...
fn build_explain_plan(inner_sql: &str, analyze: bool) -> TextResult {
    let cols = vec![
        "step".to_owned(),
        "op".to_owned(),
        "detail".to_owned(),
    ...
    let mut rows: TextRows = Vec::new();
    let push = |rows: &mut TextRows, step: usize, op: &str, detail: &str, cost: &str| {
        rows.push(vec![
            step.to_string(),
            op.to_owned(),
            detail.to_owned(),
...
fn intercept_introspection(sql: &str) -> Option<TextResult> {
    if !looks_like_introspection(sql) {
        return None;
    }
...
fn looks_like_introspection(sql: &str) -> bool {
    let s = sql.trim_start();
    starts_ci(s, "show ")
        || starts_ci(s, "select 1")
        || contains_ci(s, "@@")
        || contains_ci(s, "version()")
        || contains_ci(s, "database()")
}
    ...
    W: AsyncWrite + Send + Unpin,
{
    ...
    async fn on_execute<'a>(
        ...
        if let Some(entry) = self.stmts.get(&id) {
            ...
            let cache_scope = store_cache_scope(self.store.as_ref());
            if let Some((namespace, epoch)) = cache_scope {
                if let Some(hit) = wire_cache_get(namespace, &sql, epoch) {
                    return write_query_result(results, hit.as_ref()).await;
                }
            }
            let qr = self.store.query(&sql).await.map_err(io::Error::other)?;
            if let Some((namespace, epoch)) = cache_scope {
                if self.store.query_cache_epoch() == Some(epoch) {
                    let cached = Arc::new(qr);
                    wire_cache_insert(namespace, &sql, epoch, Arc::clone(&cached));
                    return write_query_result(results, cached.as_ref()).await;
                }
            }
            return write_query_result(results, &qr).await;
        }
    ...
    async fn on_query<'a>(
        ...
        // 4. SET statements — intercept before SQLite (SQLite doesn't understand MySQL SET syntax)
        {
            ...
            if starts_ci(trimmed, "SET ") || starts_ci(trimmed, "SET@") {
                // FOREIGN_KEY_CHECKS → PRAGMA
                let pragma = if contains_ci(trimmed, "FOREIGN_KEY_CHECKS") {
                    if contains_ci(trimmed, "=0") || contains_ci(trimmed, "= 0") {
        ...
        if is_read_only(sql) {
            ...
            write_query_result(results, &qr).await
        } else {
            let affected = self.store.exec(sql).await.map_err(io::Error::other)?;
            results
...
/// Write backend QueryResult rows as the current one-column text envelope.
...
{
    if qr.rows.is_empty() {
        return results.start(&[]).await?.finish().await;
    }
...
#[derive(Clone)]
struct WireCachedResult {
    epoch: u64,
    result: Arc<QueryResult>,
}

type WireCache = HashMap<u64, lru::LruCache<String, WireCachedResult>>;
type WireCacheShard = Mutex<WireCache>;

static WIRE_QUERY_CACHE: OnceLock<Vec<WireCacheShard>> = OnceLock::new();

fn wire_query_shards() -> &'static [WireCacheShard] {
    WIRE_QUERY_CACHE
        .get_or_init(|| {
            (0..WIRE_RESULT_CACHE_SHARDS)
...
fn wire_query_cache(sql: &str) -> &'static WireCacheShard {
    let shard = sql_shard(sql);
    &wire_query_shards()[shard]
}

fn store_cache_scope(store: &dyn Store) -> Option<(u64, u64)> {
    Some((store.query_cache_namespace()?, store.query_cache_epoch()?))
}

fn wire_cache_get(namespace: u64, sql: &str, epoch: u64) -> Option<Arc<QueryResult>> {
    let mut caches = wire_query_cache(sql).lock().ok()?;
    caches
        .get_mut(&namespace)?
        .get(sql)
...
fn wire_cache_insert(namespace: u64, sql: &str, epoch: u64, result: Arc<QueryResult>) {
    if !wire_cacheable_result(result.as_ref()) {
        return;
    }
    if let Ok(mut caches) = wire_query_cache(sql).lock() {
        let cache = caches.entry(namespace).or_insert_with(|| {
            lru::LruCache::new(
                NonZeroUsize::new(WIRE_RESULT_CACHE_CAP / WIRE_RESULT_CACHE_SHARDS).unwrap(),
            )
        });
        cache.put(sql.to_owned(), WireCachedResult { epoch, result });
    }
}

fn sql_shard(sql: &str) -> usize {
    let mut hasher = DefaultHasher::new();
    sql.hash(&mut hasher);
    (hasher.finish() as usize) % WIRE_RESULT_CACHE_SHARDS
}

fn wire_cacheable_result(result: &QueryResult) -> bool {
    if result.rows.len() > WIRE_RESULT_CACHE_MAX_ROWS {
        return false;
    }
    let bytes: usize = result.rows.iter().map(Vec::len).sum();
    bytes <= WIRE_RESULT_CACHE_MAX_BYTES
}
