...
require_contains "$package_listing" "integrations/codex/install.py" "package listing"
...
echo "4/13 codex crash-safe resume"
python3 "$repo_root/integrations/codex/hooks/test_checkpoint.py" >/dev/null

echo "5/13 init"
run_synx -f "$db" init >/dev/null

echo "6/13 remember typed memory"
...
echo "8/13 context pack with route/id/feedback"
...
echo "9/13 feedback learning path"
...
echo "10/13 prime clean project"
...
echo "11/13 fresh-context offline mode"
...
echo "12/13 doctor, safe fix, db verify"
...
echo "13/13 package install smoke"
