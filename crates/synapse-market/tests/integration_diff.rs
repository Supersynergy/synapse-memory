...
fn sqlite_range_price(
    ...
    stmt.query_map(
        rusqlite::params![ts_start, ts_end, p_lo as f64, p_hi as f64],
...
fn duckdb_range(
    ...
    let mut stmt = conn
        ...
        .unwrap();
    let rows = stmt
        .query_map(duckdb::params![ts_start, ts_end], |r| {
            Ok((
                r.get::<_, i64>(0)?,
                r.get::<_, f64>(1)? as f32,
                r.get::<_, f64>(2)? as f32,
...
fn duckdb_range_price(
    ...
    let rows = stmt
        .query_map(
            duckdb::params![ts_start, ts_end, p_lo as f64, p_hi as f64],
            |r| Ok((r.get::<_, i64>(0)?, r.get::<_, f64>(1)? as f32)),
        )
        .unwrap();
    rows.map(|r| r.unwrap()).collect()
...
fn assert_rows_match(
    ...
    assert_eq!(
        smx.len(),
        sq.len(),
        "DIFF len ticker={ticker} ts={ts_start}: smx={} sqlite={}",
        smx.len(),
        sq.len()
    );
    assert_eq!(
        smx.len(),
        dk.len(),
        "DIFF len ticker={ticker} ts={ts_start}: smx={} duckdb={}",
        smx.len(),
        dk.len()
    );
    for (bar, (sq_ts, sq_o, sq_h, sq_l, sq_c, sq_v), (dk_ts, dk_o, dk_h, dk_l, dk_c, dk_v)) in smx
        .iter()
        .zip(sq.iter())
        .zip(dk.iter())
        .map(|((a, b), c)| (a, b, c))
    {
        ...
        for (col, bv, sv, dv) in [
            ...
            assert!(
                (bv - sv).abs() <= EPS,
                "DIFF ticker={ticker} ts={} col={col}: smx={bv} sqlite={sv} delta={}",
                bar.ts,
                (bv - sv).abs()
            );
            assert!(
                (bv - dv).abs() <= EPS,
                "DIFF ticker={ticker} ts={} col={col}: smx={bv} duckdb={dv} delta={}",
                bar.ts,
                (bv - dv).abs()
            );
        }
...
fn assert_price_match(
    ...
    assert_eq!(
        smx.len(),
        sq.len(),
        "DIFF price ticker={ticker} ts={ts_start}: smx={} sqlite={}",
        smx.len(),
        sq.len()
    );
    assert_eq!(
        smx.len(),
        dk.len(),
        "DIFF price ticker={ticker} ts={ts_start}: smx={} duckdb={}",
        smx.len(),
        dk.len()
    );
    for (b, (sq_ts, sq_c), (dk_ts, dk_c)) in smx.iter().zip(sq.iter()).zip(dk.iter()) {
        assert_eq!(b.ts, *sq_ts);
        assert_eq!(b.ts, *dk_ts);
        assert!(
            (b.close - sq_c).abs() <= EPS,
            "close DIFF ticker={ticker} ts={}",
            b.ts
        );
        assert!(
            (b.close - dk_c).abs() <= EPS,
            "close DIFF ticker={ticker} ts={}",
            b.ts
        );
    }
