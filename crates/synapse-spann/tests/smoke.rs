...
fn spann_recall_10() {
    ...
    for _ in 0..trials {
        ...
        let spann_ids: std::collections::HashSet<u64> =
            index.search(&q, k, nprobe).into_iter().map(|(id, _)| id).collect();
    ...
    assert!(recall >= 0.25, "R@10={recall:.3} below 0.25 smoke threshold");
}
...
fn spann_load_roundtrip() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let cfg = SpannConfig { n_clusters: 8, dim, n_docs: n, max_iter: 20 };
    let index = SpannIndex::build(dir.path(), &docs, cfg).unwrap();
