...
fn main() {
    ...
    println!("interp:  {}ms  ({} rows out)", interp_us / 1000, interp_result.len());
    ...
    #[cfg(feature = "jit")]
    {
        ...
        println!("jit:     {}ms  ({} rows out)", jit_us / 1000, jit_result.len());
        println!("filter speedup: {:.1}×", interp_us as f64 / jit_us.max(1) as f64);
    }
    ...
    println!("interp:  {}ms  ({} groups)", interp_gb_us / 1000, interp_gb.len());
    ...
    #[cfg(feature = "jit")]
    {
        ...
        // warmup
        let _ = gb_engine.execute(gb_func, &gb_plan, &gb_rows).expect("warmup");

        let t1 = Instant::now();
        let jit_gb = black_box(gb_engine.execute(gb_func, &gb_plan, &gb_rows).expect("jit gb"));
        let jit_gb_us = t1.elapsed().as_micros();
        println!("jit:     {}ms  ({} groups)", jit_gb_us / 1000, jit_gb.len());
        println!("group_by speedup: {:.1}×", interp_gb_us as f64 / jit_gb_us.max(1) as f64);

        // second run for variance
        let t2 = Instant::now();
        let jit_gb2 = black_box(gb_engine.execute(gb_func, &gb_plan, &gb_rows).expect("jit gb2"));
        let jit_gb2_us = t2.elapsed().as_micros();
        println!("jit run2: {}ms  ({} groups)", jit_gb2_us / 1000, jit_gb2.len());
        println!("group_by speedup run2: {:.1}×", interp_gb_us as f64 / jit_gb2_us.max(1) as f64);
    ...
    println!("interp:  {}ms  ({} rows out)", interp_hj_us / 1000, interp_hj.len());
    ...
    #[cfg(feature = "jit")]
    {
        ...
        let _ = hj_engine.execute(hj_func, &hj_plan, &hj_left, &hj_right).expect("warmup");

        let t1 = Instant::now();
        let jit_hj = black_box(hj_engine.execute(hj_func, &hj_plan, &hj_left, &hj_right).expect("jit hj"));
        let jit_hj_us = t1.elapsed().as_micros();
        println!("jit:     {}ms  ({} rows out)", jit_hj_us / 1000, jit_hj.len());
        println!("hash_join speedup: {:.1}×", interp_hj_us as f64 / jit_hj_us.max(1) as f64);
    ...
    conn.execute_batch("CREATE TABLE t (k INTEGER, v INTEGER); BEGIN;").unwrap();
    {
    ...
    let _: Vec<(i64, i64)> = conn
        .prepare("SELECT k, SUM(v) FROM t GROUP BY k").unwrap()
    ...
    println!("sqlite:  {}ms  ({} groups)", sqlite_us / 1000, sqlite_gb.len());

    #[cfg(feature = "jit")]
    println!("jit vs sqlite: {:.1}×", sqlite_us as f64 / {
