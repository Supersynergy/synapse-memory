...
use synapse_pack::{
    Candidate, Kind, PackOptions, estimate_tokens, pack, pack_delta, render,
};
...
fn make_candidates(n: usize, tokens_each: usize) -> Vec<Candidate> {
    ...
    for i in 0..n {
        ...
        out.push(Candidate {
            ...
            kind: if i % 3 == 0 { Kind::KnownFact } else if i % 3 == 1 { Kind::Decision } else { Kind::File },
        });
    }
    out
}
...
fn bench_pack_scenarios(c: &mut Criterion) {
    ...
    group.bench_with_input(BenchmarkId::new("full_50_at_4000", "naive"), &cands_a, |b, cands| {
        b.iter(|| {
    ...
    group.bench_with_input(BenchmarkId::new("delta_50_skip_40", "delta"), &cands_a, |b, cands| {
        b.iter(|| {
            let opts = PackOptions {
                budget_tokens: 4000,
                header_reserve: 64,
    ...
    group.bench_with_input(BenchmarkId::new("cache_stable_50", "cs"), &cands_a, |b, cands| {
        b.iter(|| {
            let opts = PackOptions {
                budget_tokens: 4000,
                header_reserve: 64,
