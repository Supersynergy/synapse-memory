...
async fn main() -> Result<()> {
    ...
    // rustls 0.23 requires explicit CryptoProvider selection. Pick `ring`
    // (already in the dep tree). Ignore the AlreadySet error so re-init in
    // tests / re-execs is harmless.
