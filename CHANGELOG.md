...
- **Context-OS** — cross-CLI MCP server for token-budget-bounded, self-learning, verbatim context.
  ...
  - Recall quality: `recall_candidates` unions a raw-query and a high-signal-terms hybrid search, applies a term-overlap boost, and drops noise (via negativa). Noise filter = hard patterns (`is_noise`: telepathy/task-notification/status-JSON/logs/stubs) + an optional **learned logistic classifier** trained offline by `tools/ctxos/train_noise_model.py` (SuperML: LogReg exported + CatBoost validator, AUC ~0.92/0.997) and applied natively in Rust (no runtime Python, graceful when absent). `just ctxos-train` retrains from the brain + feedback. Measured: learned model drops ~10 noise docs beyond patterns on a session-heavy query while preserving known-facts. `manifest.noise_filtered` exposed.
