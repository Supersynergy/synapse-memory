...
impl QueryEngine {
    ...
    pub fn eval(&self, name: &str, events: &[ChangeEvent]) -> Result<Vec<Value>> {
        let cq = self
            .queries
            .get(name)
            .ok_or_else(|| anyhow::anyhow!("unknown CQ: {name}"))?;
        ...
        conn.execute_batch("CREATE TABLE events (ts INTEGER, op TEXT, tbl TEXT, row_json TEXT);")?;
        {
            let mut ins =
                conn.prepare("INSERT INTO events(ts,op,tbl,row_json) VALUES(?1,?2,?3,?4)")?;
