...
pub fn rewrite(sql: &str, _mode: &str) -> Result<String> {
    ...
    // WordPress expects 6 columns: Field, Type, Null, Key, Default, Extra
    ...
    if upper.starts_with("SHOW FULL COLUMNS FROM ") || upper.starts_with("SHOW COLUMNS FROM ") {
        ...
        if let Some(t) = words.last() {
            ...
            return Ok(format!(
                "SELECT \
                  name AS Field, \
                  CASE WHEN type = '' THEN 'text' ELSE lower(type) END AS Type, \
                  CASE WHEN \"notnull\" = 1 THEN 'NO' ELSE 'YES' END AS \"Null\", \
                  CASE WHEN pk = 1 THEN 'PRI' ELSE '' END AS \"Key\", \
                  dflt_value AS \"Default\", \
                  CASE WHEN pk = 1 AND lower(type) LIKE '%int%' THEN 'auto_increment' ELSE '' END AS Extra \
...
mod tests {
    ...
    #[test]
    fn test_show_columns_field_shape() {
        let out = rw("SHOW COLUMNS FROM wp_options");
        ...
        assert!(!out.contains("PRAGMA"), "should not pass raw PRAGMA: {out}");
    }

    #[test]
    fn test_show_full_columns_field_shape() {
        let out = rw("SHOW FULL COLUMNS FROM `wp_posts`");
        assert!(out.contains("Field"), "got: {out}");
        assert!(out.contains("Extra"), "got: {out}");
        assert!(out.contains("pragma_table_info"), "got: {out}");
    }

    #[test]
    fn test_describe_field_shape() {
        let out = rw("DESC wp_users");
        assert!(out.contains("Field"), "got: {out}");
        assert!(out.contains("auto_increment"), "got: {out}");
    }
