...
└────────────┬─────────────┘   └────────────┬─────────────┘   └────────────┬────────┘
     ...
     ║  tail deltas → extract prompt/reply/tool → push to synapse via `synx put` ║
     ╚═══════════════════════════════════════╤═════════════════════════════════╝
                                             ▼
                                    ┌────────────────┐
                                    │ synapse brain  │
...
| `synx put` latency | 50 ms |
| `synx search` latency | 22 ms |
...
Prereqs: `synx` CLI on PATH, Synapse daemon running (`synapsed -f ~/.synapse/brain.db &`).
...
| `SYNX_WRITE_BIN` | `synx-fast` | writer CLI for batched daemon writes |
| `SYNX_BIN` | unset | legacy override if `SYNX_WRITE_BIN` is unset |
| `TELEPATHY_SCOPE` | cwd basename | force one project scope for stored events |
| `TELEPATHY_NO_SCOPE` | unset | set `1` to disable automatic project scoping |
