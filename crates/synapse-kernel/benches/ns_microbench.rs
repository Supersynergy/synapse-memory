...
fn bench_hamming(c: &mut Criterion) {
    ...
    for &words in &[2usize, 4, 8, 16, 32] {  // 128-2048 bit
        ...
        g.bench_with_input(BenchmarkId::from_parameter(words * 64), &words, |b_, _| {
            b_.iter(|| bin_hamming::hamming_u64(std::hint::black_box(&a), std::hint::black_box(&b)));
        });
    }
    g.finish();
}
