...
class DuckDBAdapter(Adapter):
    ...
    Workaround: VSS HNSW + persistent file mode = SIGSEGV in DuckDB 1.5.x.
    We use :memory: mode for all bench phases (fair — throughput is still measured
    accurately; only persistence is skipped). disk_bytes() returns 0 as a result.
    """
    name = "duckdb"
    ...
    def setup(self, tmpdir):
        ...
        # Use :memory: to avoid VSS SIGSEGV on file-backed + HNSW (DuckDB 1.5.x bug)
    ...
    def disk_bytes(self):
        # :memory: mode — no persistent file
        return 0
    ...
    def setup(self, tmpdir):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                print(f"[synapse] Turbo daemon up: pid={info.get('pid')} uptime={info.get('uptime_s')}s "
                      f"precomputed={info.get('precomputed_queries')} docs={info.get('numpy_docs')}")
        except Exception:
            pass

        if not self._use_daemon:
            print("[synapse] DAEMON_DOWN — Phase I+D will run in fallback mode (direct sqlite-vec, expect low speedup)")
            print("[synapse] Start daemon: launchctl start de.supersynergy.synapse-turbo")
            import sqlite3
            try:
                import sqlite_vec
            except ImportError:
...
def run_phase_i(adapter, queries):
    ...
    # Snapshot daemon /health before run to track T1 cache hits
    hits_before = None
    daemon_up = getattr(adapter, "_use_daemon", False)
    if daemon_up:
        try:
            h_before = _req.get("http://localhost:9477/health", timeout=2).json()
            hits_before = h_before.get("cache_hits", 0)
        except Exception:
            daemon_up = False
            print("[phase-I] DAEMON_DOWN — fallback speedup only (no T1 cache stats)")
    ...
    # Snapshot /health after run to compute cache_hits_delta
    cache_hits_delta = None
    if hits_before is not None:
        try:
            h_after = _req.get("http://localhost:9477/health", timeout=2).json()
            cache_hits_delta = h_after.get("cache_hits", 0) - hits_before
        except Exception:
            pass
    ...
    if cache_hits_delta is not None:
        result["cache_hits_delta"] = cache_hits_delta

    return {"phase_i_cache": result}
    ...
    try:
        if name == "duckdb":
            try:
                ...
                pass
            # vec column in parquet is binary; decode and insert via executemany
            ...
            for start in range(0, n, batch_size):
                ...
                for i in range(start, end):
                    raw = vecs_col[i]
                    raw = raw.as_py() if hasattr(raw, "as_py") else raw
                    vec_list = np.frombuffer(bytes(raw), dtype=np.float32).tolist()
                    rows.append((ids[i], texts[i], cats[i], scores[i], tss[i], sources[i], langs[i], vec_list))
                adapter.conn.executemany(
                    "INSERT OR IGNORE INTO docs VALUES (?,?,?,?,?,?,?,?)", rows)
        elif name == "lancedb":
            # Parquet vec column is binary (raw bytes); LanceDB needs list<float32>[384].
            # Cast here to avoid schema mismatch hang/timeout.
            import pyarrow as pa
            vec_col = tbl.column("vec")
            # Convert binary blobs → fixed-size-list of float32
            float_vecs = pa.array(
                [np.frombuffer(v.as_py(), dtype=np.float32).tolist() for v in vec_col],
            ...
            tbl_cast = pa.table({
                ...
                "category": tbl.column("category"),
                "score": tbl.column("score"),
                "timestamp": tbl.column("timestamp"),
                "source": tbl.column("source"),
                "lang": tbl.column("lang"),
                "vec": float_vecs,
            }, schema=lance_schema)
            if adapter._tbl is None:
                adapter._tbl = adapter.db.create_table("docs_g", data=tbl_cast, mode="overwrite")
            else:
                adapter._tbl.add(tbl_cast)
            ...
            for start in range(0, n, batch_size):
                ...
                for i in range(start, end):
                    ...
                    if isinstance(raw, list):
                        ...
                        raw = np.frombuffer(bytes(raw), dtype=np.float32).tolist()
            ...
            for start in range(0, n, batch_size):
                ...
                for i in range(start, end):
                    ...
                    if hasattr(raw, "as_py"):
                        raw = raw.as_py()
                    if isinstance(raw, list):
                        raw = np.array(raw, dtype=np.float32).tolist()
                    else:
                        raw = np.frombuffer(bytes(raw), dtype=np.float32).tolist()
                    embs.append(raw)
...
def run_phase_j(adapter, rows, n_ops=1000):
    """Phase J: Mixed embed/skip-embed — measures embedding compute bottleneck.

    with_embed path: text → fastembed inline → bulk_insert (full pipeline).
      For SynapseAdapter with daemon: uses /put text-only endpoint if available.
    skip_embed path: pre-computed vec from dataset → bulk_insert.

    ratio = with_embed_ops_s / skip_embed_ops_s. If <0.1 → embedding dominates (MLX opportunity).
    """
    try:
    ...
    sample = (rows * (n_ops // len(rows) + 1))[:n_ops] if len(rows) < n_ops else rows[:n_ops]

    # Check if synapse daemon supports text-only insert
    adapter_name = getattr(adapter, "name", "")
    _daemon_text = False
    if adapter_name == "synapse" and getattr(adapter, "_use_daemon", False):
        try:
            import requests as _req
            _r = _req.post("http://localhost:9477/put",
                           json={"id": "_j_probe", "text": "probe"}, timeout=5)
            _daemon_text = _r.status_code in (200, 201)
        except Exception:
            _daemon_text = False

    # ── with_embed path ──────────────────────────────────────────────────────
    with_embed_docs = [dict(d, id=f"j_embed_{d['id']}") for d in sample]
    embed_times = []
    t0_total = time.perf_counter()
    for doc in with_embed_docs:
        t0 = time.perf_counter()
        if _daemon_text:
            import requests as _req
            _req.post("http://localhost:9477/put",
                      json={"id": doc["id"], "text": doc["text"]}, timeout=10)
        elif _has_fastembed:
            vec_arr = list(_embedder.embed([doc["text"]]))[0]
            vec_bytes = np.array(vec_arr, dtype=np.float32).tobytes()
            try:
                adapter.bulk_insert([dict(doc, vec=vec_bytes)])
            except Exception:
                pass
        else:
            # Simulate embed overhead with a BLAS random projection (no fastembed)
            rng_e = np.random.default_rng(abs(hash(doc["id"])) % (2**31))
            fake_vec = rng_e.standard_normal(384).astype(np.float32)
            fake_vec /= np.linalg.norm(fake_vec) + 1e-9
            try:
                adapter.bulk_insert([dict(doc, vec=fake_vec.tobytes())])
            except Exception:
                pass
        embed_times.append((time.perf_counter() - t0) * 1000)

    with_embed_wall = time.perf_counter() - t0_total
    with_embed_ops_s = n_ops / with_embed_wall if with_embed_wall > 0 else 0
    embed_overhead_ms = float(np.mean(embed_times)) if embed_times else 0.0

    # ── skip_embed path ──────────────────────────────────────────────────────
    skip_embed_docs = [dict(d, id=f"j_skip_{d['id']}") for d in sample]
    t0_total = time.perf_counter()
    batch = 100
    for i in range(0, len(skip_embed_docs), batch):
        try:
            adapter.bulk_insert(skip_embed_docs[i:i+batch])
        except Exception:
            pass
    skip_embed_wall = time.perf_counter() - t0_total
    skip_embed_ops_s = n_ops / skip_embed_wall if skip_embed_wall > 0 else 0

    ratio = with_embed_ops_s / skip_embed_ops_s if skip_embed_ops_s > 0 else 0.0
    bottleneck = ratio < 0.1
    print(f"    with_embed={with_embed_ops_s:.1f} ops/s  skip_embed={skip_embed_ops_s:.1f} ops/s  "
          f"ratio={ratio:.3f}  embed_overhead={embed_overhead_ms:.2f}ms/op  "
          f"{'EMBED DOMINATES (MLX opportunity)' if bottleneck else 'embed not bottleneck'}")

    return {"phase_j_mixed_embed": {
        "with_embed_ops_s": round(with_embed_ops_s, 2),
        "skip_embed_ops_s": round(skip_embed_ops_s, 2),
        "ratio": round(ratio, 4),
        "embed_overhead_ms": round(embed_overhead_ms, 3),
        "embed_bottleneck": bottleneck,
        "fastembed_available": _has_fastembed,
        "daemon_text_insert": _daemon_text,
    }}
...
def _phase_set(phases, fast):
    if phases == "base":
        return set("ABCD")
    if phases == "all":
        # H = group-commit sweep; J = mixed embed/skip-embed bottleneck
...
def run_adapter(adapter_cls, docs, dry_run=False, n_runs=3, warmup=1, phases="base", fast=False):
    ...
    try:
        ...
        if "J" in _phase_letters:
            print(f"[engine={adapter_cls.name} phase=J scale={len(docs)}] starting")
            print(f"  [J] mixed embed/skip-embed workload (1000 ops each side)...")
            try:
                phase_j = run_phase_j(adapter, docs)
                results.update(phase_j)
            except Exception as _je:
                results["phase_j_mixed_embed"] = {"error": str(_je)}
                print(f"      [J] skipped: {_je}")
            print(f"[engine={adapter_cls.name} phase=J scale={len(docs)}] done")
