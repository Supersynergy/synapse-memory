...
fn test_dsl_drought_buy() {
    ...
    assert!(matches!(p, Pattern::DroughtBuy { quiet_days: 180, min_value: 250000.0 }));
}

#[test]
fn test_dsl_insider_cluster() {
    let p = parse_pattern("InsiderCluster(k=3, w=30)").unwrap();
    assert!(matches!(p, Pattern::InsiderCluster { k: 3, window_days: 30 }));
}
...
fn test_dsl_volume_spike() {
    ...
    assert!(matches!(p, Pattern::VolumeSpike { multiplier: _, window_bars: 20 }));
...
#[test]
fn test_dsl_then() {
    let p = parse_pattern("InsiderCluster(k=3, w=30) THEN VolumeSpike(mult=2.5, w=20) within 14").unwrap();
