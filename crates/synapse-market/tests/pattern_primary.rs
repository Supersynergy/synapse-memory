...
fn test_fda_meeting_plus_s3_fires() {
    ...
    engine.on_event(
        ...
        &Event::FdaMeeting {
            ticker: t.into(),
            ts: 0,
            meeting_type: "PDUFA".into(),
        },
    );
    let m = engine.on_event(
        t,
        &Event::S3Filing {
            ticker: t.into(),
            ts: 20 * SECS,
            raise_size: 50_000_000.0,
        },
    );
    assert_eq!(m.len(), 1);
...
fn test_spinoff_approaching_fires() {
    ...
    let m = engine.on_event(
        t,
        &Event::SpinoffAnnouncement {
            ticker: t.into(),
            ts: now,
            distribution_date: distrib,
        },
    );
    assert_eq!(m.len(), 1);
    assert_eq!(m[0].pattern_id, id);
...
fn test_float_squeeze_fires() {
    let mut engine = FsmEngine::new();
    let id = engine.register(Pattern::FloatSqueeze {
        si_min: 20.0,
        ctb_bps_min: 50,
    });
...
fn test_float_squeeze_no_fire_below_threshold() {
    let mut engine = FsmEngine::new();
    engine.register(Pattern::FloatSqueeze {
        si_min: 20.0,
        ctb_bps_min: 50,
    });
...
fn test_compose_filing13d_and_float_squeeze() {
    ...
    assert!(
        m.iter().any(|x| x.pattern_id == id),
        "And(13D, FloatSqueeze) should fire"
    );
}
...
fn test_compose_then_congress_then_squeeze() {
    ...
    assert!(
        m.iter().any(|x| x.pattern_id == id),
        "Then(CongressTrade, FloatSqueeze) should fire"
    );
}
