...
use crate::{OhlcvRow, error::Result};
...
pub fn ingest(conn: &Connection, symbol: &str, rows: &[OhlcvRow]) -> Result<()> {
    if rows.is_empty() {
        return Ok(());
    }
...
pub fn fetch_range(conn: &Connection, symbol: &str, start: i64, end: i64) -> Result<Vec<OhlcvRow>> {
    let t = table_name(symbol);
...
#[allow(dead_code)]
pub fn fetch_all(conn: &Connection, symbol: &str) -> Result<Vec<OhlcvRow>> {
    let t = table_name(symbol);
