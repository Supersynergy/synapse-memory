...
use crate::quant::{QuantizedTokenVec, max_sim_i8, quant_i8};
use crate::{ColbertEmbedder, max_sim};
use anyhow::Result;
use rusqlite::{Connection, params};

pub type RerankHit = (i64, f32);
...
impl<'a> ColbertStore<'a> {
    ...
    pub fn colbert_rerank_i8(
        ...
        let query_q: Vec<QuantizedTokenVec> = query_vecs.iter().map(|v| quant_i8(v)).collect();
        let mut scores: Vec<RerankHit> = candidates
            .iter()
            .filter_map(|&doc_id| {
    ...
    fn load_vecs_i8(&self, doc_id: i64) -> Result<Vec<QuantizedTokenVec>> {
    ...
    pub fn colbert_rerank(&self, query_text: &str, candidates: &[i64]) -> Result<Vec<RerankHit>> {
        let query_vecs = self.emb.embed_query(query_text)?;
        let mut scores: Vec<RerankHit> = candidates
            .iter()
            .filter_map(|&doc_id| {
