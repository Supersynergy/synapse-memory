...
fn make_event(ts: i64, level: u8, side: Side, op: Op, px: f32, qty: f32) -> BookEvent {
    BookEvent { ts, level, side, op, px, qty }
}
...
fn naive_replay(events: &[BookEvent], ts: i64) -> (f32, f32) {
    ...
    let best_bid = bids.iter().filter(|&&(p, q)| p > 0.0 && q > 0.0)
        .map(|&(p, _)| p).fold(f32::NEG_INFINITY, f32::max);
    let best_ask = asks.iter().filter(|&&(p, q)| p > 0.0 && q > 0.0)
...
fn events_between_range() {
    let tmp = NamedTempFile::new().unwrap();
    let mut store = BookStore::open(tmp.path()).unwrap();
    let events: Vec<BookEvent> = (0..100).map(|i| make_event(
