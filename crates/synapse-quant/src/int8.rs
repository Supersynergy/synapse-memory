...
impl Quantizer for Int8 {
    ...
    fn encode(&self, v: &[f32]) -> Result<Int8Vec, QuantError> {
        if v.len() != self.dim { return Err(QuantError::DimMismatch { expected: self.dim, actual: v.len() }); }
        let max_abs = v.iter().fold(0f32, |m, x| m.max(x.abs())).max(1e-10);
        let codes: Vec<i8> = v.iter().map(|x| (x / max_abs * 127.0).round().clamp(-128.0, 127.0) as i8).collect();
        Ok(Int8Vec { scale: max_abs, codes })
    }

    fn decode(&self, e: &Int8Vec) -> Result<Vec<f32>, QuantError> {
        if e.codes.len() != self.dim { return Err(QuantError::DimMismatch { expected: self.dim, actual: e.codes.len() }); }
        Ok(e.codes.iter().map(|c| (*c as f32) * e.scale / 127.0).collect())
    }
    ...
    fn distance(&self, a: &Int8Vec, b: &Int8Vec) -> f32 {
        ...
        let dot: i32 = a.codes.iter().zip(b.codes.iter()).map(|(x, y)| (*x as i32) * (*y as i32)).sum();
...
mod tests {
    ...
    fn distance_orders() {
        let q = Int8::new(3);
        let mut a = vec![1.0_f32, 0.0, 0.0]; norm(&mut a);
        let mut b = vec![0.9, 0.1, 0.0]; norm(&mut b);
        let mut c = vec![0.0, 1.0, 0.0]; norm(&mut c);
