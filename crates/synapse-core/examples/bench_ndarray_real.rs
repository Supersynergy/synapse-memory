...
fn main() {
    ...
    let n = search.len();
    println!("  Direct connection: {} vectors loaded in {:.0}ms", n, load_ms);
    ...
    for i in 0..25 {
        ...
        if i >= 5 { t_ndarray.push(ms); }
        if i == 5 { println!("  results: {:?}", r.iter().take(3).collect::<Vec<_>>()); }
    }
    ...
    println!("  p50={}ms  p95={}ms", format!("{:.2}", p50), format!("{:.2}", p95));
